...
import br.ufscar.sorocaba.appia.services.talk.ReceiveApplicationLayer;

public class ReceiveExecutor {

	public static void main(String[] args) {
		Layer[] queue = { new UdpSimpleLayer(), new ReceiveApplicationLayer() };

		QoS myQoS = createQoS("talk_stack", queue);
		Channel channel = myQoS.createUnboundChannel("talk_channel");
		
	    bind((ReceiveApplicationLayer) queue[1], channel);
		start(channel);
